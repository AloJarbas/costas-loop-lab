...
def _text(x: float, y: float, text: str, *, size: int = 16, anchor: str = "start", fill: str = "#111827", weight: str = "normal") -> str:
...
def render_costas_demo_svg(trace: LoopTrace, *, output: str | Path, title: str = "QPSK carrier recovery: coarse estimate, then Costas tracking") -> None:
    ...
    for _, points, left, top, panel_w, panel_h, color, subtitle in scatters:
        cx = left + panel_w / 2
        cy = top + panel_h / 2
        scale = 88.0
        parts.append(f'<rect x="{left:.1f}" y="{top:.1f}" width="{panel_w:.1f}" height="{panel_h:.1f}" rx="18" fill="#ffffff" stroke="#e2e8f0"/>')
        parts.append(_text(cx, top + 26, subtitle, size=15, anchor="middle", fill="#334155", weight="700"))
        parts.append(_line(left + 26, cy, left + panel_w - 26, cy, stroke="#cbd5e1"))
        parts.append(_line(cx, top + 36, cx, top + panel_h - 22, stroke="#cbd5e1"))
        for point in points:
    ...
    parts.append(_text(90, 800, f"coarse phase estimate from first 64 symbols: {trace.coarse_phase:+.3f} rad", size=14, fill="#475569"))
    parts.append('</svg>')
    output.write_text("\n".join(parts) + "\n")
...
def render_offset_sweep_svg(rows: list[dict[str, float]], *, output: str | Path, title: str = "Costas loop offset sweep") -> None:
    ...
    parts.append(_text(right - 180, top + 24, "raw", size=14, fill="#ef4444", weight="700"))
    parts.append(_text(right - 180, top + 46, "after coarse phase", size=14, fill="#f59e0b", weight="700"))
    parts.append(_text(right - 180, top + 68, "after Costas tracking", size=14, fill="#2563eb", weight="700"))
    parts.append(_text((left + right) / 2, height - 26, "carrier offset (rad/sample)", anchor="middle", size=16, fill="#374151"))
    parts.append(_text(24, (top + bottom) / 2, "RMS distance to nearest QPSK corner", anchor="middle", size=16, fill="#374151"))
    parts.append('</svg>')
    output.write_text("\n".join(parts) + "\n")
